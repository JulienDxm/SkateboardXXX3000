...
class MovuinoDataSet():
    ...
    def run(self):
        for k in range(self.nb_line):
            ...
            if k < self.nb_line-1:
                ...
                posAngX = self.gyroscope[k][0]*pas + self.posAng[k][0]  # 360/2np.pi
                posAngY = self.gyroscope[k][1]*pas + self.posAng[k][1]
                posAngZ = self.gyroscope[k][2]*pas + self.posAng[k][2]
                self.posAng.append(np.array([posAngX, posAngY, posAngZ]))

                vx = self.acceleration[k][0]*pas + self.velocity[k][0]  # 360/2np.pi
                vy = self.acceleration[k][1]*pas + self.velocity[k][1]
                vz = self.acceleration[k][2]*pas+ self.velocity[k][2]
                self.velocity.append(np.array([vx, vy, vz]))

                posX = self.velocity[k][0]*pas + self.pos[k][0]  # 360/2np.pi
                posY = self.velocity[k][1]*pas + self.pos[k][1]
                posZ = self.velocity[k][2]*pas + self.pos[k][2]
    ...
    def PlotImage(self):
        ...
        theta.plot(self.time, self.posAng[:, 0], color="r")
        theta.plot(self.time, self.posAng[:, 1], color="green")
        theta.plot(self.time, self.posAng[:, 2], color="blue")
        ...
        plt.legend(handles=[patchX, patchY, patchZ], loc="center right", bbox_to_anchor=(2.5,2.4),ncol=1)
        plt.show()
